...
# Lunar Lander with "Play Again" option
...
while True:
    ...
    if height > 0.01:    
        ...
        display_final()
        stopped = True
        if key == pygame.K_y:  #if user presses 'y' key to play again,
            fuel = 5000.0       # reset everything
            velocity = -100.0
            height = 2000
            thrust = 0
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT or (key == pygame.K_n and stopped): 
            sys.exit()
            ...
            if held_down:
                ...
                if myThrottle.rect.centery > 500:
                    myThrottle.rect.centery = 500
        elif event.type  == pygame.KEYDOWN:  # gey keypresses, only when game has ended
            if stopped:
                key = event.key
            else:
                key = ''
